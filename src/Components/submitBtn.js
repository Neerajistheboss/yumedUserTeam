...
function SubmitBtn(props) {
	...
	function login() {
		axios
			.post('http://13.233.58.33:5000/api/v1/auth/login', {
				email: props.email,
	...
	function register() {
		...
		axios
			.post('http://13.233.58.33:5000/api/v1/auth/register', {
				name: props.name,
				email: props.email,
				password: props.password,
				phone: props.phone,
		...
		axios
			...
			.then(function (response) {
				...
				if (!response.data.success) {
					...
					window.location.href = `http://13.233.58.33:5000/api/v1/payment/${orderId}`
				}
			})
	}
